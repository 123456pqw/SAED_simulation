...
csv_file = '/share/SAED_analysis/utils/data.csv'  # 替换为您的 CSV 文件路径
...
# 单视角网络
class SVCNN(Model):
    def __init__(self, name, nclasses=7, pretraining=True, cnn_name='resnet18'):
...
# 多视角网络
class MVCNN(Model):
    def __init__(self, name, svcnn, nclasses_h1=7, nclasses_h2=222, cnn_name='resnet18', num_views=2):
        super(MVCNN, self).__init__(name)
        
        # 共享组件
        self.features = svcnn.features
        self.task2_feature = svcnn.task2_feature
        self.num_views = num_views
        self.log_var_h1 = svcnn.log_var_h1  # log(σ^2)
        self.log_var_h2 = svcnn.log_var_h2
...
if __name__ == "__main__":
    # 单视角测试
    svcnn = SVCNN(name='resnet18')
    x = torch.randn(4, 3, 224, 224) 
    y1, y2 = svcnn(x)
    print(f"SVCNN输出: task1={y1.shape}, task2={y2.shape}") 
    
    # 多视角测试
    mvcnn = MVCNN(name='resnet188', svcnn=svcnn, num_views=2)
    x_mv = torch.randn(8, 3, 224, 224)  # 4个样本，每个2视角
    y1_mv, y2_mv = mvcnn(x_mv)
    print(f"MVCNN输出: task1={y1_mv.shape}, task2={y2_mv.shape}") 